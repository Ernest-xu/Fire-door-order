...
<body>	
	...
	$(function(){
			...
			$('#dcl_contract_list').datagrid({
				...
				{field:'action1',title:'',width:80,align:'center',
					...
					formatter:function(value,row,index){
							var g = '<a href="javascript:void(0)" onclick="dcl_discussion_onclick('+row.id+')">讨论</a> ';
					...
					formatter:function(value,row,index){
							var g = '<a href="javascript:void(0)" onclick="dcl_submit_onclick('+row.id+')">提交</a> ';
	...
	<div id="dcl_html_dlg"  class="easyui-dialog" style="width:780px;height:580px;padding:center;" modal=true closed="true"></div>
	<script type="text/javascript">	
	var cl_id;
	var html_id;
	...
	var dcl_r=0;
		...
		//讨论
		function dcl_discussion_onclick(index){
			html_id=index;
			$('#dcl_html_dlg').dialog({
				title:'编辑内容',
				width:780,
				height:580,
				modal:true,
				content:"<iframe scrolling='auto' frameborder='0' src='dagang_chat_dome.php' style='width:100%;height:100%;display:block;'></iframe>"
			});
		$('#dcl_html_dlg').dialog('open');
			
		}
		//提交
		function dcl_submit_onclick(index){
			$.messager.confirm('确认','确定提交吗?',function(r){
					if (r){
						$.ajax({
							type: "POST",
							contentType: "application/json; charset=utf-8",	
							url: "dagang_contract/update_stage.php",
							dataType: "json",				
